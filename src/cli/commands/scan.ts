...
export function registerScanCommand(program: Command): void {
  program
    .command('scan')
    .description(
      'Scan .claude/agents and .claude/skills for agents/skills not yet tracked or already '
        + 'in --remote, and print a ready-to-edit `push --new` command for each',
    )
    ...
    .action(async (options: { remote: string }) => {
      ...
      if (candidates.length === 0) {
        console.log('No new agents/skills found in .claude/agents or .claude/skills.');
        return;
      }
