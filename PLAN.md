...
- [x] Tag-based bulk pull, in the app
      Done — Browse now has a second chip row for every `tags.{roles,teams,stacks}` value present across the catalog (e.g. "stack: python"), filtered independently of the existing Kind row. A "Pull all (N)" button appears whenever the current filter (kind + tag + search combined) has 1+ not-yet-pulled or update-available artifacts, and pulls them all sequentially with per-item progress in its own label. Deliberately excludes `edited_locally`/`both_changed` entries from bulk actions — those still require the existing per-artifact confirmation in Detail, never something a bulk click silently overwrites.
