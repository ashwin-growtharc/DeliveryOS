...
stacked in one continuous vertical scroll regardless of artifact kind,
and `.detail-readme` never actually parsed markdown at all (`textContent`
+ `white-space: pre-wrap`, so headers/bold/tables/links/code-fences all
showed up as literal `#`/`**`/`|` characters) -- both raised directly by
the user after seeing `growtharc-react-vite-starter`'s real Detail view.

- [x] **Vendored `marked`** as a static asset
      (`src-tauri/spike-ui/vendor/marked.min.js`), generated by a new
      `scripts/generate-vendored-marked.mjs` (a straight copy of
      `marked`'s own real UMD browser build -- no esbuild rebuild
      needed, unlike `generate-vendored-libraries.mjs`), wired into
      `prebuild`/`pretypecheck`/`pretest` alongside the other three
      `generate-vendored-*.mjs` scripts. Loaded via a plain `<script>`
      tag before `app.js` -- `spike-ui` has no build step at all.
- [x] **`renderMarkdownToSandboxedIframe(container, markdownText)`**
      (`app.js`) -- a real, deliberate security decision, not
      incidental: `marked` does not sanitize by design, so its renderer
      is overridden to ESCAPE raw HTML rather than pass it through
      (reusing the existing `escapeHtml` helper), and the parsed output
      is wrapped into a small self-contained document (same CSP-meta-tag
      convention as `compile.ts`'s `injectPreviewCsp`, `img-src data:`
      only) and rendered inside a `sandbox="allow-scripts"` iframe --
      never injected into the host page's DOM directly, matching this
      codebase's existing "sandbox anything less-trusted" discipline. A
      small inline content-height-reporter script (simpler than
      `injectContentHeightReporter`'s React-aware version -- just a
      `ResizeObserver` on `document.body`) sizes the iframe to its real
      content, no inner scrollbar. Falls back to plain preformatted text
      if `marked` throws. New `.markdown-body` typographic styling
      (EB Garamond headings, IBM Plex Sans body, JetBrains Mono code,
      real tables/blockquotes/links) lives inside the iframe's own
      injected `<style>`, derived from tokens already in `:root`.
- [x] **Detail's sections split into top-level tabs** (`#detail-tabs-row`,
      reusing the exact `.tab-row`/`.tab`/`.tab.active` convention
      already used for preview-variant switching), built dynamically per
      artifact via a shared `detailTabState`/`refreshDetailTabs()`
      controller -- each section flips its own key once it knows
      whether it applies (two, Preview/Configuration, are known
      synchronously from the manifest; Documentation/Design/Components/
      Routes resolve via RPC) and the controller decides the actual tab
      set and active panel. Collapses to no tab row at all when at most
      one section applies -- the common case, most artifacts just have
      a README. **Documentation unified into one tab regardless of
      kind** -- a `kind: backend-plugin` artifact's README used to
      render via a separate code path (`renderInstallParamsSection`)
      from every other kind's (`renderGenericReadmeSection`); both
      fetched the exact same `README.md` for the exact same purpose, so
      `renderDocumentationTab` now does both README.md AND (new)
      GUIDELINES.md's own full prose -- previously never shown anywhere
      in the app at all, only its extracted structured tokens were.
      `renderTemplateSection` split into `renderDesignAndComponentsSections`
      (Design: tokens/type-scale/layout-rules; Components: the live
      grid), sharing the one `artifact.parseGuidelines` fetch. The
      provenance badge moved out of the backend-plugin section into
      Detail's always-visible identity/status cluster (status info, not
      documentation).
- [x] **Verified with a real, in-browser simulation** (Playwright +
      `playwright-core`'s already-established `chromium.launch({channel:
      'msedge'})` technique, serving the actual unmodified
      `index.html`/`app.js`/`style.css`/vendored `marked.min.js` via a
      throwaway static server, stubbing only the native Tauri bridge --
      `window.__TAURI__.core.invoke`/`window.DeliveryOS.call` -- with
      canned sidecar responses): a rich fake artifact (install_params +
      README + GUIDELINES.md + components + routes all present) shows
      the correct 5 tabs and switches between them one panel at a time;
      a plain README-only fake artifact correctly shows NO tab row,
      Documentation rendered directly instead; a deliberate
      `<script>alert(1)</script>`/`<img onerror>` payload embedded in
      the fake README renders as inert, escaped text and never executes
      (`window.__XSS_FIRED__` stays `false`). Real dogfood against the
      live catalog's actual content (`growtharc-react-vite-starter`'s
      README, `design-kit`'s/the starter kit's own GUIDELINES.md)
      confirms every markdown feature they actually use -- including
      inline code spans containing literal `<Outlet/>`/`useRouteError()`
      -- renders correctly (real `<h1>`/`<h2>`/`<table>`/`<pre>`/`<ul>`
