/* Verbatim copy of `packages/shared/src/utils/chalk-colors.ts` — the real
 * implementation behind `chalkColors` from `@kortix/shared`. It is pure data
 * (a string hash → HSL triple) with zero runtime dependencies, so copying it
 * in preserves the exact colour every avatar has in the product. Nothing here
 * is invented. */
