...
### Font loading
**Vendored, not fetched.** `vendor/fonts.css` is generated by
`scripts/generate-vendored-fonts.mjs` from the pinned `@fontsource` packages
and shipped with the app, so it renders identically offline. It used to load
from Google Fonts, which meant all three faces silently fell back to system
fonts without a network — the app looked different depending on connectivity.

Eleven faces, latin subset only:
- EB Garamond: 400, 500, 600, plus 400 italic
- IBM Plex Sans: 400, 500, 600, 700
- JetBrains Mono: 400, 500, 600

`npm run ui:fonts` loads the real page with all external requests blocked and
asserts every face still resolves.
...
Colour was tokenised from the start; everything else was written out by hand,
screen by screen, for thirty phases. The result was 13 distinct font sizes
(four of them half-pixel), 44 padding combinations, 17 off-grid spacing values
and five spellings of three font stacks. Each choice was locally reasonable;
the accumulation is what "the UI is inconsistent" actually meant.

These are the tokens. `npm run lint:css` fails the build on anything that
bypasses them.
...
| Token | Size | Use |
|---|---|---|
| `--text-2xs` | 11px | Badges and micro-labels only |
| `--text-xs` | 12px | Hints, captions, meta |
| `--text-sm` | 13px | The de-facto body size; labels, buttons |
| `--text-base` | 14px | Body copy |
| `--text-lg` | 18px | Card and section titles |
| `--text-xl` | 20px | Page titles |
| `--text-2xl` | 22px | The brand mark |

A field label must never be larger than the heading of the card it lives in.

### Text colour

Three real per-theme colours. Never express muted text as an `opacity` on
`--ink` — that composites below the contrast floor, and because every rule
picks its own number the same role ends up with several different values.

| Token | Light | Dark | Use |
|---|---|---|---|
| `--text-primary` | 11.2:1 | 12.9:1 | Body copy, values, names |
| `--text-secondary` | 7.4:1 | 6.4:1 | Things you read: labels, hints, summaries |
| `--text-tertiary` | 5.7:1 | 4.9:1 | Things you glance at: counts, meta, captions |

`opacity` survives only for genuine transparency — disabled states, hover
fades, decorative icons.

### Spacing

Multiples of 4: `--space-1` (4px) through `--space-8` (32px), plus
`--space-05` (2px) as a deliberate half-step for badge and pill padding, which
genuinely needs sub-4px in a dense tool UI.

Negative offsets and values above 32px are exempt — those are optical
corrections measured against a real edge, and page-level layout, not rhythm.
...
| `--radius-md` | 8px | Buttons, inputs, nav items — the default |
| `--radius-lg` | 16px | Cards |
| `--radius-xl` | 20px | Large surfaces |
| `--radius-pill` | 500px | Fully-rounded pills |
| `--motion-fast` | `.15s ease` | Ordinary state changes |
| `--motion-reveal` | `.2s ease-out` | A larger reveal |
| `--shadow-1` / `--shadow-2` | | Resting / raised |
| `--z-sticky` / `--z-overlay` / `--z-toast` | 10 / 1000 / 2000 | |

### Loading

Use a **skeleton** shaped like the content it stands in for, not a spinner.
Three sections loading at once produce three competing spinners and no sense
of what is happening; skeletons recede and nothing jumps when the real content
arrives. A button that is working keeps its own label and gains a spinner —
never swap the label for "Working…", or several in-flight buttons become
indistinguishable.

Skeleton animation is disabled entirely under `prefers-reduced-motion`, never
merely slowed.

### Empty vs. error

They are different states and must look different. `EmptyState` means "nothing
here yet". A failed fetch is an `ErrorState`: it names what failed, shows the
engine's own message verbatim rather than a paraphrase, and offers a retry
...
Starter Kits      -> straight to kind:"template"
Backend Plugins   -> straight to kind:"backend-plugin"
Settings
────────────────  (divider)
Scan              -> detect installable artifacts in the current project
Add New           -> the wizard (from Scan) or a flat form (from here)
```
...
- **Contrast**: Every text rule clears WCAG AA (4.5:1) in **both** themes.
  This is enforced, not asserted: `test/unit/uiContrast.test.ts` resolves the
  token graph and composites any surviving `opacity` exactly as a browser
  would, and fails the build otherwise. `npm run ui:contrast` prints the full
  report. It was previously asserted and untrue — 46 of 200 theme-by-surface
  checks failed, worst of all the primary nav tabs at 2.75:1, because muted
  text was an `opacity` on the ink colour rather than its own token.
