# PLAN.md archive — the full phase-by-phase log

The blow-by-blow record of Phases 0–32, moved out of `PLAN.md` on 2026-08-28.

`PLAN.md` is meant to answer "where are we and what's next". It had grown to
1,499 lines because every phase kept its full implementation narrative — the
later ones running to 80, 140, even 212 lines apiece, including individual bug
fixes. That is a changelog, not a plan, and `CHANGELOG.md` already covers the
same ground in the form people actually reach for.

Kept intact here rather than deleted: the reasoning behind a decision is often
the only place a constraint was ever written down.
