...
| ~~CLI `check-updates --apply` hits **all** artifacts; sidecar requires an `id`~~ — **fixed.** The CLI now takes an optional `[id]` and threads it to the `onlyId` the engine had accepted all along; project-wide is still the default with no id, but it is now a choice. Worth keeping as the record of a drift class the capability manifest cannot catch: both surfaces declared the operation and their risk flags agreed — they differed in **granularity**, and nothing compares argument shapes | `src/cli/commands/checkUpdates.ts` vs `src/sidecar.ts:457` |
